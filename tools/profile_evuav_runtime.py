...
import ev_flying_runtime_common as runtime_common
...
def parse_args() -> argparse.Namespace:
    ...
    runtime_common.add_common_args(
        ...
        REPO_ROOT / "analysis_outputs" / "evuav_runtime_v1" / "grid_mamba",
    )
...
def get_single_knn_cache_key(batch: Mapping[str, Any]) -> str | None:
...
def clone_sample(sample: Mapping[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "knn_cache_key": sample.get("knn_cache_key"),
    }
...
def main() -> int:
    ...
    def infer(item: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        with torch.inference_mode():
            logits, _ = model(points, knn_cache_key=knn_cache_key)
