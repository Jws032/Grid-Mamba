...
CONDA_ENVS_ROOT = Path(
    os.environ.get(
        "CONDA_ENVS_ROOT",
        str(Path(sys.executable).resolve().parents[2]),
    )
)
...
JOBS: Dict[str, Dict[str, Any]] = {
    ...
    "grid_mamba": {
        ...
        "module": "tools.runtime.dataset.profile_ev_flying_runtime",
...
def run_job(job_id: str, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if not args.skip_gpu_idle_check:
        check_gpu_idle()
    command: List[str] = [str(job["python"])]
    if job.get("module"):
        command.extend(["-m", str(job["module"])])
    else:
        command.append(str(job["script"]))
    command.extend([
        "--device",
        args.device,
        "--warmup",
